...
func (wh *websocketHandler) SetUpgrade(u *websocket.Upgrader) {
	wh.upgrader = u
}

func (wh *websocketHandler) SetAuth(f func(token string) (userID string, ok bool)) {
	wh.calcUserIDFunc = f
}
...
func (s *pushHandler) SetAuth(f func(r *http.Request) bool)  {
	s.authFunc = f
}
